...
data = np.array([[t/(t+f)*100 for t,f in model_data] for model_data in medqa])
...
for i, model in enumerate(models):
    ...
    for j in range(len(variations)):
        perturbed_true, perturbed_false = medqa[i][j]
...
excel_filename = "medqa_permutation_results.xlsx"
excel_df.to_excel(excel_filename, index=False, sheet_name="Permutation Test Results")
plot_data(data, ci_lower, ci_upper, adjusted_p_values, "LLM Performance Across Perturbations (MedQA)", "medqa_perturbation_graph.png")
